...
public class UMLDiagram extends JPanel {
    ...
    public UMLDiagram(String title, List<ClassDescriptor> umlClasses) {
    ...
    private void drawClass(Graphics2D graphics, ClassDescriptor umlClass, Point position) {
        ...
        int fieldHeight = drawFields(graphics, umlClass.getFields(), x, fieldStartY, width);

        int methodStartY = fieldStartY + fieldHeight;
        int methodHeight = drawMethods(graphics, umlClass.getMethods(), x, methodStartY, width);

        graphics.setColor(Color.BLACK);
        graphics.drawRect(x, y, width, height + fieldHeight + methodHeight);
    }
